...
class SecureUserInfoResource {

    val logger: Logger = LoggerFactory.getLogger(SecureUserInfoResource::class.java)
    val client: HttpClient = HttpClient.newHttpClient()

    @GetMapping
    fun secure(@RequestHeader headers: HttpHeaders): ResponseEntity<String> {

        val authServer = URI("http://localhost:8080/realms/api-authorization/protocol/openid-connect/userinfo")
        ...
        return if (response.statusCode() == 200) {
            ResponseEntity.ok("User Loggued")
        } else {
